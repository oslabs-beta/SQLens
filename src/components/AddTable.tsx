...
import { ImPlus } from 'react-icons/im';
import useStore from '../store';
import { ADD_TABLE_MUTATION } from '../utilities/queries';
...
const AddTable = ({
  ...
  const handleCheckClick = async () => {
    try {
      const cleanedLabel = editedLabel.trim().replace(/[^A-Za-z0-9_]/g, '_');
      await addTable({ variables: { tableName: cleanedLabel } });
      const newTable = { name: cleanedLabel, columns: [], foreignKeys: [] };
      setTables([...tables, newTable]);
      setIsEditing(false);
      setEditedLabel('');
    } catch (error) {
